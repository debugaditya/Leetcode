...
public:
    int earliestFinishTime(vector<int>& lt, vector<int>& ld, vector<int>& wt, vector<int>& wd) {
        ...
        for(int i=0;i<lt.size();i++) if(wet[0]<=lt[i]) ans=min(ans,lt[i]+ld[i]);
        for(int i=0;i<wt.size();i++) if(let[0]<=wt[i]) ans=min(ans,wt[i]+wd[i]);
        for(int i=0;i<lt.size();i++) if(lt[i]<=wet[0]) ans=min(ans,wet[0]+ld[i]);
        for(int i=0;i<wt.size();i++) if(wt[i]<=let[0]) ans=min(ans,let[0]+wd[i]); 
        return ans;
    }
};
